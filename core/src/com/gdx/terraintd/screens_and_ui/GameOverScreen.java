...
public class GameOverScreen implements Screen {
    ...
    public GameOverScreen(final TerrainTDGame game, StatsManager statsManager, GameSaveManager gameSaveManager) {
        this.game = game;
        int finalScore = statsManager.score;
        stage = new Stage(new ScreenViewport());
        Gdx.input.setInputProcessor(stage);

        BitmapFont titleFont = GameConstants.getFont(100, Color.CORAL);
        BitmapFont scoreFont = GameConstants.getFont(80, Color.CHARTREUSE);
        BitmapFont font = GameConstants.getFont(70, Color.WHITE);

        backgroundTexture = new Texture(Gdx.files.internal("images/gameoverscreen.png"));
        backgroundSprite = new Sprite(backgroundTexture);
        backgroundSprite.setSize(Gdx.graphics.getWidth(), Gdx.graphics.getHeight());

        batch = new SpriteBatch();

        Label.LabelStyle titleStyle = new Label.LabelStyle(titleFont, Color.CORAL);
        Label.LabelStyle scoreStyle = new Label.LabelStyle(scoreFont, Color.CHARTREUSE);
